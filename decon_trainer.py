...
CONFIG = {
    ...
    # Training
    'num_state_action_pairs_per_difficulty': 500000,
    'evaluate_every_num_epochs': 5,
    'entropy_threshold': 0.03,
    'num_epochs_per_difficulty': 5,
    'batch_size': 256,
...
class DeConTrainer:
    def __init__(self, env, grid_size, num_tile_types, device):
        ...
        # Initialize networks
        self.decon = DeConNetwork(grid_size, num_tile_types).to(device)
        
        # Initialize optimizers
        self.con_optimizer = Adam(
            self.decon.con.parameters(),
            lr=CONFIG['learning_rate_construction']
        )
        self.con_loss = nn.CrossEntropyLoss()
        
        self.de_optimizer = Adam(
            self.decon.de.parameters(),
    ...
    def save_checkpoint(self, difficulty_pct):
        ...
        checkpoint = {
            'construction_net_state_dict': self.decon.con.state_dict(),
            'destruction_net_state_dict': self.decon.de.state_dict(),
            'construction_optimizer_state_dict': self.con_optimizer.state_dict(),
            'destruction_optimizer_state_dict': self.de_optimizer.state_dict(),
            'difficulty': difficulty_pct
        }
        torch.save(checkpoint, os.path.join(CONFIG['decon_dir'], f'decon_difficulty_{difficulty_pct:.2f}.pt'))
    ...
    def generate_random_trajectory(self, difficulty, grid_size):
        ...
        for _ in range(trajectory_length):
            ...
            next_state_pos_dict, reward, done, truncated, info = self.unwrapped.env.step(action)
    ...
    def generate_decon_trajectory(self, difficulty, grid_size):
        ...
        for _ in range(trajectory_length):
            ...
            for a in possible_actions:
                ...
                entropy = self.decon.get_actual_con_entropy(state_tensor)
    ...
    def train(self):
        ...
        for difficulty in CONFIG['difficulty_levels']:
            ...
            curr_patience = 0
            while solve_pct < CONFIG['solve_pct_thresh'] or mean_entropy > CONFIG['inference_entropy_thresh']: # and curr_patience <= CONFIG['patience']:
                curr_patience += 1
            
                trajectories = []
                num_state_action_pairs = 0
                while num_state_action_pairs <= CONFIG['num_state_action_pairs_per_difficulty']:
                    trajectory, avg_entropy_during_traj_sampling = self.generate_decon_trajectory(difficulty, CONFIG['grid_size'])
                ...
                for epoch in range(CONFIG['num_epochs_per_difficulty']):
                    ...
                    for batch_states, batch_actions, batch_entropies in dataloader:
                        ...
                        pred_entropies, pred_actions, actual_entropies = self.decon(batch_states.squeeze(1), which='both')

                        # import pdb; pdb.set_trace()
                        loss_de = self.de_loss(pred_entropies, actual_entropies.unsqueeze(1))
                    ...
                    print(f'Epoch [{epoch+1}/{CONFIG["num_epochs_per_difficulty"]}], Con Loss: {avg_loss_con:.4f}, De Loss: {avg_loss_de:.4f}, Entropy: {avg_entropy:.4f}, Correct {avg_correct_pct:.4f}, Time: {epoch_time:.2f}s')
                    
                    # Every 100 epochs, evaluate the network's performance
                    if (epoch + 1) % CONFIG['evaluate_every_num_epochs'] == 0:
                        construction_net_copy = copy.deepcopy(self.decon.con)
                ...
                # After using the dataset and dataloader, explicitly delete them
                del dataset
                del dataloader
                import gc
            ...
            metrics_per_difficulty['de_losses'][difficulty] = de_losses
        ...
        metrics_json = {
            "metrics_per_difficulty": {
                ...
                "de_losses": {str(k): [float(x) for x in v] for k, v in metrics_per_difficulty['de_losses'].items()},
        ...
        # Save to JSON file
        with open(os.path.join(CONFIG['decon_dir'], 'decon_metrics.json'), 'w') as f:
        ...
        for difficulty in CONFIG['difficulty_levels']:
            ...
            plt.plot(de_losses, label=f'De Loss (diff={difficulty})')
        plt.title('Construction and Destruction Losses')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(CONFIG['decon_dir'], 'decon_curriculum_metrics.png'))
        plt.close()
...
DeConTrainer(env, CONFIG['grid_size'], CONFIG['num_tile_types'], 'cuda').train()