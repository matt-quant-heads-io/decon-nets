...
def gen_random_map(random, width, height, prob):
    map = np.random.choice(list(prob.keys()),size=(height,width),p=list(prob.values())).astype(np.uint8)
    return map
...
def get_range_reward(new_value, old_value, low, high):
    # print(f"new_value: {new_value}, old_value: {old_value}, low: {low}, high: {high}")
